            ...
            <tbody>
                <?php if(count($reservations) > 0): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="font-weight: 600; color: var(--slate-900);"><?= htmlspecialchars($res['reserved_by']) ?></div>
                        </td>
                        <td>
                        ...
                        <td>
                            ...
                            <div style="font-size: 0.8rem; color: var(--slate-500);">
                                <?= date('h:i A', strtotime($res['start_time'])) ?> - <?= date('h:i A', strtotime($res['end_time'])) ?>
                            </div>
                        </td>
                        <?php if($is_admin): ?>
                        <td>
                            <div style="display: flex; align-items: center; gap: 0.5rem;">
                                <div style="width: 24px; height: 24px; background-color: var(--slate-200); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 600;">
                        ...
                        <td>
                            <?php
                                ...
                                $icon = 'fa-clock';
                                if($res['status_name'] == 'Approved') { $status_class = 'badge-success'; $icon = 'fa-check'; }
                                if($res['status_name'] == 'Rejected' || $res['status_name'] == 'Cancelled') { $status_class = 'badge-danger'; $icon = 'fa-xmark'; }
                            ?>
                            <span class="badge <?= $status_class ?>">
                                <i class="fa-solid <?= $icon ?>" style="margin-right: 0.25rem;"></i> <?= $res['status_name'] ?>
                            </span>
                        </td>
                        <?php if($is_admin): ?>
                        <td>
                            ...
                            <div style="display: flex; gap: 0.5rem;">
                                <a href="update_status.php?id=<?= $res['reservation_id'] ?>&status=Approved" class="btn btn-sm btn-primary" style="padding: 0.35rem 0.75rem;" title="Approve">
                                    <i class="fa-solid fa-check"></i>
                                </a>
                                <a href="update_status.php?id=<?= $res['reservation_id'] ?>&status=Rejected" class="btn btn-sm btn-danger" style="padding: 0.35rem 0.75rem;" title="Reject">
                        ...
                        <td>
                            ...
                            <div style="display: flex; gap: 0.5rem;">
                                <a href="edit.php?id=<?= $res['reservation_id'] ?>" class="btn btn-sm btn-outline" title="Edit/Reschedule">
                                    <i class="fa-solid fa-pen-to-square"></i> Edit
                                </a>
                                <a href="cancel.php?id=<?= $res['reservation_id'] ?>" class="btn btn-sm btn-danger" title="Cancel Booking" onclick="return confirm('Are you sure you want to cancel this reservation? This action cannot be undone.')">
