...
// upcoming_count: different current-time functions per driver
if ($driver === 'oci') {
    $stmt = $pdo->query("SELECT COUNT(*) FROM Reservation WHERE start_time >= SYSTIMESTAMP");
} else {
    // SQLite and others
...
<div class="grid grid-cols-3 gap-6 mb-4">
    <div class="card">
        <div class="card-body" style="display: flex; align-items: center; justify-content: space-between;">
            <div>
                <p style="color: var(--slate-500); font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Total
                    Venues</p>
                <h2 style="font-size: 2rem; font-weight: 700; color: var(--slate-900);"><?= $venue_count ?></h2>
            </div>
            <div
                style="width: 56px; height: 56px; background-color: var(--primary-50); border-radius: var(--radius-xl); display: flex; align-items: center; justify-content: center; color: var(--primary-600);">
    ...
    <div class="card">
        <div class="card-body" style="display: flex; align-items: center; justify-content: space-between;">
            <div>
                <p style="color: var(--slate-500); font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Pending
                    Requests</p>
                <h2 style="font-size: 2rem; font-weight: 700; color: var(--slate-900);"><?= $pending_count ?></h2>
            </div>
            <div
                style="width: 56px; height: 56px; background-color: #FFFBEB; border-radius: var(--radius-xl); display: flex; align-items: center; justify-content: center; color: #D97706;">
    ...
    <div class="card">
        <div class="card-body" style="display: flex; align-items: center; justify-content: space-between;">
            <div>
                <p style="color: var(--slate-500); font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">
                    Upcoming Bookings</p>
                <h2 style="font-size: 2rem; font-weight: 700; color: var(--slate-900);"><?= $upcoming_count ?></h2>
            </div>
            <div
                style="width: 56px; height: 56px; background-color: #ECFDF5; border-radius: var(--radius-xl); display: flex; align-items: center; justify-content: center; color: #059669;">
    ...
    <div class="table-container">
        <table>
            ...
            <tbody>
                ...
                // Build recent reservations query with driver-specific LIMIT/FETCH
                ...
                <?php if (count($recent_reservations) > 0): ?>
                    <?php foreach ($recent_reservations as $res): ?>
                        <?php $res = array_change_key_case($res, CASE_LOWER); ?>
                        <tr>
                            <td><span
                            ...
                            <td>
                                ...
                                if ($res['status_name'] == 'Denied' || $res['status_name'] == 'Cancelled')
